...
/**
 * An alternate JSON exposure for Script definitions.
 */
public class ScriptDef {
     ...
     * Constructor.
     *
     * @param script The script definition being wrapped.
     */
    public ScriptDef(Script script) {
        super();
        number = script.getNumber();
        code = script.getCode();
        altCodes = script.getAltCodes();
        name = script.getName();
        javaName = (script.getUnicodeScript() != null) ? script.getUnicodeScript().name() : null;
        aliases = script.getAliases();
    }
}