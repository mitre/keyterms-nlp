...
public class TextTransformer_Fra
        extends TextTransformer {

    private static final Language SRC_LANG = Language.FRENCH;
    private static final Language TRG_LANG = Language.ENGLISH;

    public TextTransformer_Fra() {
        this(SRC_LANG,TRG_LANG);
    }

    public TextTransformer_Fra(Language source, Language target) {
        super(source,target);
    }

    @Override
    protected void initializeNormalizer() {
        normalizer = new Normalizer_Fra();
    }

    @Override
    protected void initializeStemmer() {
        stemmer = new Stemmer_Fra();
    }
