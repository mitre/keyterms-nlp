...
public class TestEncodingAnalyzer
    ...
    public TestEncodingAnalyzer() {
        super(Bags.staticSet(byte[].class), Bags.staticSet(TextInfo.ENCODING, TextInfo.LENGTH), false, false);
    }

    @Override
    protected void _analyze(Object input, Consumer<Analysis> collector) {
        int length = (input instanceof byte[])
                ? Encoding.decode((byte[])input, Encoding.UTF8).length()
                : Strings.toString(input).length();
