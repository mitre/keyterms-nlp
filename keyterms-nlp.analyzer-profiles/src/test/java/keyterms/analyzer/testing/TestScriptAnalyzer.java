...
public class TestScriptAnalyzer
        extends Analyzer {

    public TestScriptAnalyzer() {
        super(Bags.staticSet(CharSequence.class), Bags.staticSet(TextInfo.SCRIPT), false, false);
    }
    ...
    protected void _analyze(Object input, Consumer<Analysis> collector) {
        ...
        for (Map.Entry<String, String> entry : TestData.LANGUAGE_PHRASES.entrySet()) {
            if (entry.getValue().equals(text)) {
                script = Script.byCode(entry.getKey().replaceFirst("([^-]*)-(.*)", "$2"));
            }
        }
