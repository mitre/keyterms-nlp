...
public class JsonConfigParser
    ...
    public List<Keyed<String, List<String>>> parse(CharSequence text) {
        ...
        if (Strings.hasText(text)) {
            JsonObject configObject = GsonUtil.toObject(JsonObject.class, text);
            if (configObject != null) {
                configObject.entrySet().forEach((entry) -> {
                    ...
                    if (!value.isJsonNull()) {
                        if (value.isJsonArray()) {
                            ...
                            for (int a = 0; a < array.size(); a++) {
                                valueList.add(array.get(a).getAsString());
                            }
                        } else {
                    ...
                    valueList = valueList.stream()
                            .map(Strings::replaceSystemTokens)
                            .collect(Collectors.toList());
                    entries.add(new Keyed<>(key, valueList));
                });
            }
        }
        return entries;
